...
$(document).ready(function() {
	...
	$('#activity').click(function(e) {
		...
		if($("#activity .alerts-num").length > 0){
			$.ajax({
				url: mt.getBaseURL('reset-notifications'),
	...
	$('#logout a').click(function(e) {
		...
		// ask verification
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> Завершить работу <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
			content : "Выйти из личного кабинета и завершить текущий сеанс?",
			buttons : '[Нет][Да]'
